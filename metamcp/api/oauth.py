...
async def oauth_callback(
    ...
    try:
        ...
        if not code or not state:
            return OAuthCallbackResponse(
                success=False, error="Missing required OAuth parameters"
            )
        ...
        return OAuthCallbackResponse(
            success=True, user=oauth_user, access_token=access_token
        )
        ...
        return OAuthCallbackResponse(success=False, error="Internal server error")
...
async def get_agent_oauth_status(
    ...
    try:
        ...
        is_authenticated = await oauth_manager.validate_agent_session(
            agent_id, provider
        )
        ...
        raise HTTPException(status_code=500, detail="Failed to get agent OAuth status")
...
async def revoke_agent_oauth_session(
    ...
    try:
        ...
        return {"message": f"OAuth session revoked for agent {agent_id} ({provider})"}

    except Exception as e:
        logger.error(f"Failed to revoke agent OAuth session: {e}")
        raise HTTPException(
            status_code=500, detail="Failed to revoke agent OAuth session"
        )
...
async def get_agent_oauth_token(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to get agent OAuth token")
...
async def fastmcp_agent_authenticate(
    agent_id: str, provider: str, requested_scopes: list[str] | None = None
) -> dict[str, str]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to initiate agent OAuth flow"
        )
...
async def fastmcp_agent_session_status(agent_id: str, provider: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get agent session status"
        )


@router.post("/fastmcp/agent/{agent_id}/token")
async def fastmcp_agent_token_exchange(
    agent_id: str, provider: str, authorization_code: str, state: str
) -> dict[str, Any]:
    ...
    try:
        ...
        oauth_user = await oauth_manager.handle_callback(
            provider=provider, code=authorization_code, state=state
        )

        # Verify this is an agent authentication
        if not oauth_user.is_agent:
            raise HTTPException(
                status_code=400, detail="This endpoint is for agent authentication only"
            )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to exchange authorization code for token"
        )
...
async def get_oauth_configuration() -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to get OAuth configuration")
