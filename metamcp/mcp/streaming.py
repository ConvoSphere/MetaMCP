...
class StreamOperation:
    ...
    async def receive_chunk(self) -> StreamChunk | None:
        ...
        try:
            chunk = await asyncio.wait_for(
                self._receive_queue.get(), timeout=self.config.timeout
            )
    ...
    def is_active(self) -> bool:
        ...
        return not self._closed and self.status in [
            StreamStatus.ACTIVE,
    ...
    async def _send_loop(self) -> None:
        ...
        while stream.is_active:
            try:
                ...
                chunk = await asyncio.wait_for(stream._send_queue.get(), timeout=1.0)
...
class StreamingProtocol:
    ...
    async def send_streaming_resource_reading(
        self, uri: str, operation_id: str
    ) -> AsyncGenerator[StreamChunk, None]:
        """Send streaming resource reading."""
        stream = await self.stream_manager.create_stream(
            StreamType.RESOURCE_READING, operation_id, {"uri": uri}
        )

        # Send initial chunk
        await stream.send_chunk({"type": "resource_reading_start", "uri": uri})
