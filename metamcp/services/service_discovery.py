...
class ServiceDiscovery:
    ...
    async def get_services_by_type(
        ...
        for service_id in self.service_registries[service_type]:
            ...
            if service_info and (not healthy_only or service_info.status == ServiceStatus.HEALTHY):
    ...
    async def discover_services(self) -> Dict[str, Any]:
        ...
        healthy_services = len([s for s in all_services if s.status == ServiceStatus.HEALTHY])
        ...
        async with httpx.AsyncClient(timeout=5) as client:
            for service_info in self.registered_services.values():
                ...
                try:
                    ...
                    logger.warning(
                        f"Health check failed for {service_info.id}: {e}"
                    )
                    await self.update_service_status(
                        service_info.id, ServiceStatus.UNHEALTHY
                    )
    ...
    async def _store_service_in_cache(self, service_info: ServiceInfo) -> None:
        ...
        try:
            ...
            service_data["created_at"] = service_info.created_at.isoformat() if service_info.created_at else None
            service_data["updated_at"] = service_info.updated_at.isoformat() if service_info.updated_at else None
            service_data["last_heartbeat"] = service_info.last_heartbeat.isoformat() if service_info.last_heartbeat else None
