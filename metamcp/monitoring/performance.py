...
class PerformanceMonitor:
    ...
    def _setup_prometheus_metrics(self) -> None:
        ...
        self.cpu_gauge = Gauge(
            "metamcp_cpu_usage_percent",
            "CPU usage percentage",
            ["component"]
        )
        self.memory_gauge = Gauge(
            "metamcp_memory_usage_bytes",
            "Memory usage in bytes",
            ["component"]
        )
        self.memory_percent_gauge = Gauge(
            "metamcp_memory_usage_percent",
            "Memory usage percentage",
            ["component"]
        )
        self.disk_gauge = Gauge(
            "metamcp_disk_usage_bytes",
            "Disk usage in bytes",
            ["component"]
        )
        self.disk_percent_gauge = Gauge(
            "metamcp_disk_usage_percent",
        ...
        self.network_bytes_sent = Counter(
            "metamcp_network_bytes_sent_total",
            "Total bytes sent",
            ["component"]
        )
        self.network_bytes_recv = Counter(
            "metamcp_network_bytes_recv_total",
        ...
        self.active_requests = Gauge(
            "metamcp_active_requests",
            "Number of active requests"
        )
        
        # Error metrics
        self.error_counter = Counter(
            "metamcp_errors_total",
            "Total errors",
            ["error_type", "component"]
        )
        self.error_rate = Gauge(
            "metamcp_error_rate_percent",
            "Error rate percentage"
        )
        
        # Performance metrics
        self.response_time_summary = Summary(
            "metamcp_response_time_seconds",
            "Response time summary",
            ["method", "path"]
        )
        self.request_rate = Gauge(
            "metamcp_request_rate_per_second",
            "Requests per second"
        )
    ...
    async def _collect_system_metrics(self) -> None:
        ...
        try:
            ...
            if time_diff > 0:
                sent_rate = (network_bytes_sent - self.last_network_io.bytes_sent) / time_diff
                recv_rate = (network_bytes_recv - self.last_network_io.bytes_recv) / time_diff
            else:
                sent_rate = 0
                recv_rate = 0
    ...
    async def _update_prometheus_metrics(self) -> None:
        ...
        try:
            ...
            self.memory_percent_gauge.labels(component="system").set(latest_metrics.memory_percent)
            self.disk_gauge.labels(component="system").set(latest_metrics.disk_used)
            self.disk_percent_gauge.labels(component="system").set(latest_metrics.disk_usage_percent)
        ...
        try:
            ...
            if request_metrics.status_code >= 400:
                ...
                self.error_counter.labels(
                    error_type=f"http_{request_metrics.status_code}",
                    component="api"
                ).inc()
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "alerts": {
                ...
                "response_time_high": latest_metrics.response_time_avg > self.response_time_threshold,
                "error_rate_high": latest_metrics.error_rate > self.error_rate_threshold,
            }
        }
    ...
    def get_request_analytics(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            "status_code_distribution": self._get_status_code_distribution(recent_requests),
        }
    
    def _get_status_code_distribution(self, requests: List[RequestMetrics]) -> Dict[str, int]:
