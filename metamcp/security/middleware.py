...
class APIKeyManager:
    ...
    def add_api_key(self, key: str, name: str, permissions: List[str],
                    rate_limit: int = 1000, expires_at: Optional[int] = None):
        """Add a new API key."""
        self.api_keys[key] = {
            "name": name,
    ...
    def get_key_rate_limit(self, key: str) -> Optional[int]:
...
class SecurityMiddleware:
    ...
    async def __call__(self, request: Any, call_next):
        ...
        try:
            ...
            if self.config.enable_request_signing:
                ...
                # Verify timestamp
                if not self.request_signer.verify_timestamp(timestamp_int, self.config.max_timestamp_drift):
                    return self._create_security_error_response(
                ...
                if not self.request_signer.verify_signature(
                    request.method, str(request.url.path), body, signature, timestamp_int
                ):
                    return self._create_security_error_response(
    ...
    def _create_security_error_response(self, message: str, status_code: int, 
                                      details: Dict[str, Any] = None) -> Any:
