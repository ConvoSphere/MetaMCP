...
from ..exceptions import ValidationError, EmbeddingError
    ...
    try:
        if not tool_description or not isinstance(tool_description, str):
            raise EmbeddingError(
                text=str(tool_description),
                reason="Tool description must be a non-empty string"
            )
        
        if not isinstance(tool_capabilities, list):
            raise EmbeddingError(
                text=str(tool_capabilities),
                reason="Tool capabilities must be a list"
            )
        ...
        raise EmbeddingError(
            text=str(tool_description),
            reason=f"Failed to create embedding text: {str(e)}"
        )
...
def validate_tool_schema(tool_data: Dict[str, Any]) -> bool:
    ...
    optional_fields = [
        "category", "capabilities", "security_level", "schema",
    ...
    for field in required_fields:
        ...
        if not tool_data[field] or not isinstance(tool_data[field], str):
            raise ValidationError(
                field=field,
                reason="Required field must be a non-empty string"
            )
    ...
    if "capabilities" in tool_data:
        if not isinstance(tool_data["capabilities"], list):
            raise ValidationError(
                field="capabilities",
                reason="Capabilities must be a list"
            )
    
    if "security_level" in tool_data:
        if not isinstance(tool_data["security_level"], int) or tool_data["security_level"] < 0:
            raise ValidationError(
                field="security_level",
                reason="Security level must be a non-negative integer"
            )
    ...
    if "schema" in tool_data:
        if not isinstance(tool_data["schema"], dict):
            raise ValidationError(
                field="schema",
                reason="Schema must be a dictionary"
            )
...
def sanitize_input(data: Any, max_length: int = 1000) -> Any:
    ...
    if isinstance(data, str):
        ...
        return [sanitize_input(item, max_length) for item in data[:10]]  # Limit list size
