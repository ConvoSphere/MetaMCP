...
class APIVersionManager:
    ...
    def add_version(self, version_info: VersionInfo):
        ...
        self.logger.info(
            f"Added API version: {version_info.version} ({version_info.status.value})"
        )
    ...
    def is_version_deprecated(self, version: str) -> bool:
        ...
        now = datetime.utcnow()
        return (
            version_info.deprecation_date and now > version_info.deprecation_date
    ...
    def check_compatibility(
        ...
        if server_info.min_client_version:
            if self._parse_version(client_version) < self._parse_version(
                server_info.min_client_version
            ):
                return (
                    False,
                    f"Client version {client_version} is too old. Minimum required: {server_info.min_client_version}",
                )

        if server_info.max_client_version:
            if self._parse_version(client_version) > self._parse_version(
                server_info.max_client_version
            ):
                return (
                    False,
                    f"Client version {client_version} is too new. Maximum supported: {server_info.max_client_version}",
                )

        return True, "Compatible"

    def get_deprecation_warning(self, version: str) -> str | None:
...
class APIVersionMiddleware:
    ...
    async def __call__(self, request: Any, call_next):
        ...
        response.headers["X-API-Latest-Version"] = (
            self.version_manager.get_latest_version()
        )
    ...
    def _create_version_error_response(
        self, message: str, status_code: int, supported_versions: list[str] = None
    ) -> Any:
