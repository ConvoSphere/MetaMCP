...
class ErrorHandler:
    ...
    def create_error_context(self, request: Any = None, user_id: Optional[str] = None) -> ErrorContext:
        """Create error context from request."""
        context = ErrorContext()
        
        if request:
            context.client_ip = getattr(request.client, 'host', None) if request.client else None
    ...
    def create_error_response(self, error_details: ErrorDetails) -> Dict[str, Any]:
        ...
        response = {
            "error": {
                ...
                "correlation_id": error_details.context.correlation_id if error_details.context else None,
    ...
    def handle_exception(self, exc: Exception, context: Optional[ErrorContext] = None) -> ErrorDetails:
    ...
    def get_error_statistics(self) -> Dict[str, Any]:
        ...
        for error in self.error_reports:
            ...
            stats["by_code"][error.error_code] = stats["by_code"].get(error.error_code, 0) + 1
...
class ErrorRecoveryHandler:
    ...
    def create_retry_response(self, error_details: ErrorDetails, attempt_count: int) -> Dict[str, Any]:
...
def handle_request_error(exc: Exception, request: Any = None, user_id: Optional[str] = None) -> Dict[str, Any]:
