...
class TestRunner:
    ...
    def run_performance_tests(self) -> Dict:
        ...
        cmd = [
            ...
            "-m", "performance or load or stress or benchmark"
        ]
    ...
    def run_security_scanning(self) -> Dict:
        ...
        cmd = [
            ...
            "-r", "metamcp/",
            "-f", "json",
            "-o", "test-results/security-scan.json"
        ]
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if slow_tests:
            recommendations.append(f"Optimize slow test suites: {', '.join(slow_tests)}")
        
        # Check coverage
        if "coverage_analysis" in self.test_results and self.test_results["coverage_analysis"]["success"]:
                    ...
                    if "TOTAL" in line:
                        try:
                            ...
                            if coverage_percentage < 80:
                                recommendations.append(f"Increase test coverage (currently {coverage_percentage:.1f}%)")
                        except (ValueError, IndexError):
                            pass
                        break
        
        # Check security issues
        if "security_scanning" in self.test_results and self.test_results["security_scanning"]["success"]:
            try:
                ...
                if security_file.exists():
                    with open(security_file) as f:
                        ...
                        if security_data.get("results"):
                            high_severity = [r for r in security_data["results"] if r.get("issue_severity") == "HIGH"]
                            if high_severity:
