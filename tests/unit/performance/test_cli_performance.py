...
class TestCLIPerformance:
    ...
    def test_cli_initialization_performance(self, temp_project):
        ...
        try:
            ...
            print(
                f"CLI initialization: {execution_time:.3f}s, Memory: {memory_usage:.2f}MB"
            )
    ...
    def test_environment_validation_performance(self, temp_project):
        ...
        try:
            ...
            print(
                f"Environment validation: {execution_time:.3f}s, Memory: {memory_usage:.2f}MB"
            )
    ...
    def test_docker_status_performance(self, temp_project):
        ...
        try:
            ...
            with patch("subprocess.run") as mock_run:
                ...
                print(
                    f"Docker status: {execution_time:.3f}s, Memory: {memory_usage:.2f}MB"
                )
    ...
    def test_large_env_file_performance(self, temp_project):
        ...
        try:
            ...
            large_env_content = []
            for i in range(1000):
                large_env_content.append(
                    f"LARGE_VAR_{i}=very_long_value_with_many_characters_{i}"
                )
            ...
            print(
                f"Large env file validation: {execution_time:.3f}s, Memory: {memory_usage:.2f}MB"
            )
    ...
    def test_concurrent_operations_performance(self, temp_project):
        ...
        try:
            ...
            assert (
                execution_time < 5.0
            )  # Should handle concurrent operations in under 5 seconds
            assert memory_usage < 50.0  # Should use less than 50MB additional memory
            assert all(op == 0 for op in operations)  # All operations should succeed

            print(
                f"Concurrent operations: {execution_time:.3f}s, Memory: {memory_usage:.2f}MB"
            )
    ...
    def test_memory_leak_detection(self, temp_project):
        ...
        try:
            ...
            print(
                f"Memory leak test: {memory_increase:.2f}MB increase after 100 operations"
            )
    ...
    def test_command_parsing_performance(self, temp_project):
        ...
        try:
            ...
            print(
                f"Command parsing: {execution_time:.3f}s for {len(commands)} commands"
            )
    ...
    def test_file_operations_performance(self, temp_project):
        ...
        try:
            ...
            assert (
                execution_time < 2.0
            )  # Should perform file operations in under 2 seconds
            assert memory_usage < 10.0  # Should use less than 10MB additional memory

            print(
                f"File operations: {execution_time:.3f}s, Memory: {memory_usage:.2f}MB"
            )
...
class TestCLIResourceUsage:
    ...
    def test_disk_io_performance(self, temp_project):
        ...
        try:
            ...
            print(
                f"Disk I/O: {read_bytes / 1024:.1f}KB read, {write_bytes / 1024:.1f}KB written"
            )
