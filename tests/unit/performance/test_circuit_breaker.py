...
class TestCircuitBreaker:
    ...
    def test_circuit_half_open_after_timeout(self):
        ...
        config = CircuitBreakerConfig(
            name="test", failure_threshold=1, recovery_timeout=1  # 1 second timeout
        )
        cb = CircuitBreaker(config)
    ...
    def test_circuit_closes_after_success_threshold(self):
        ...
        config = CircuitBreakerConfig(
            name="test", failure_threshold=1, recovery_timeout=1, success_threshold=2
        )
        cb = CircuitBreaker(config)
    ...
    def test_circuit_reopens_on_failure_in_half_open(self):
        ...
        config = CircuitBreakerConfig(
            name="test", failure_threshold=1, recovery_timeout=1
        )
        cb = CircuitBreaker(config)
...
class TestCircuitBreakerManager:
    ...
    def test_get_circuit_breaker_with_config(self):
        ...
        config = CircuitBreakerConfig(
            name="custom", failure_threshold=5, recovery_timeout=30
        )
