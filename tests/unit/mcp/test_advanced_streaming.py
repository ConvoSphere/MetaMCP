...
class TestStreamOperation:
    ...
    async def test_receive_chunk(self):
        ...
        chunk = StreamChunk(
            stream_id="test-stream", chunk_id="test-chunk", data={"received": "data"}
        )
    ...
    async def test_receive_all_chunks(self):
        ...
        chunk1 = StreamChunk(
            stream_id="test-stream", chunk_id="chunk1", data={"chunk": 1}
        )
        chunk2 = StreamChunk(
            stream_id="test-stream", chunk_id="chunk2", data={"chunk": 2}, is_final=True
        )
...
class TestStreamManager:
    ...
    async def test_create_stream(self):
        ...
        stream = await manager.create_stream(
            StreamType.TOOL_EXECUTION, "test-operation", {"test": "metadata"}
        )
    ...
    async def test_get_stream(self):
        ...
        created_stream = await manager.create_stream(
            StreamType.TOOL_EXECUTION, "test-operation"
        )
...
class TestStreamingProtocol:
    ...
    async def test_broadcast_event_stream(self):
        ...
        with patch.object(protocol.stream_manager, "create_stream") as mock_create:
            ...
            await protocol.broadcast_event_stream(
                "test_event", {"data": "test"}, "test-operation"
            )
    ...
    async def test_get_streaming_statistics(self):
        ...
        # Create some streams
        await protocol.create_bidirectional_stream(StreamType.TOOL_EXECUTION, "op1")
        await protocol.create_bidirectional_stream(StreamType.RESOURCE_READING, "op2")
...
class TestStreamingIntegration:
    ...
    async def test_complete_streaming_workflow(self):
        ...
        # Create a bidirectional stream
        bidirectional = await protocol.create_bidirectional_stream(
            StreamType.TOOL_EXECUTION, "test-workflow", {"workflow": "test"}
        )
    ...
    async def test_multiple_concurrent_streams(self):
        ...
        for i in range(3):
            stream = await protocol.create_bidirectional_stream(
                StreamType.TOOL_EXECUTION, f"operation-{i}"
            )
