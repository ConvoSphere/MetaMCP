...
class LoadTestRunner:
    ...
    async def run_basic_load_test(self) -> LoadTestResult:
        ...
        try:
            for i in range(self.config.total_requests):
                ...
                try:
                    if method == "GET":
                        response = await self.http_client.get(f"{self.config.base_url}{path}")
                    elif method == "POST":
        ...
        logger.info(f"Basic load test completed: {result.requests_per_second:.2f} RPS, {result.error_rate:.2f}% errors")
    ...
    async def run_stress_test(self) -> LoadTestResult:
        ...
        while current_users <= max_users:
            ...
            if result.error_rate > self.config.error_threshold or result.avg_response_time > self.config.max_response_time:
    ...
    async def run_spike_test(self) -> LoadTestResult:
        ...
        # Run normal load
        normal_result = await self._run_load_test_with_config(normal_config, "Spike Test - Normal")
        
        # Run spike load
        spike_result = await self._run_load_test_with_config(spike_config, "Spike Test - Spike")
        
        # Run normal load again
        recovery_result = await self._run_load_test_with_config(normal_config, "Spike Test - Recovery")
        ...
        combined_result = LoadTestResult(
            ...
            total_requests=normal_result.total_requests + spike_result.total_requests + recovery_result.total_requests,
            successful_requests=normal_result.successful_requests + spike_result.successful_requests + recovery_result.successful_requests,
            failed_requests=normal_result.failed_requests + spike_result.failed_requests + recovery_result.failed_requests,
            total_response_time=normal_result.total_response_time + spike_result.total_response_time + recovery_result.total_response_time,
            avg_response_time=(normal_result.avg_response_time + spike_result.avg_response_time + recovery_result.avg_response_time) / 3,
            min_response_time=min(normal_result.min_response_time, spike_result.min_response_time, recovery_result.min_response_time),
            max_response_time=max(normal_result.max_response_time, spike_result.max_response_time, recovery_result.max_response_time),
            p95_response_time=max(normal_result.p95_response_time, spike_result.p95_response_time, recovery_result.p95_response_time),
            p99_response_time=max(normal_result.p99_response_time, spike_result.p99_response_time, recovery_result.p99_response_time),
            requests_per_second=(normal_result.requests_per_second + spike_result.requests_per_second + recovery_result.requests_per_second) / 3,
            error_rate=(normal_result.error_rate + spike_result.error_rate + recovery_result.error_rate) / 3,
            concurrent_users=spike_config.users,
            status_codes={**normal_result.status_codes, **spike_result.status_codes, **recovery_result.status_codes},
    ...
    async def _run_load_test_with_config(self, config: LoadTestConfig, test_name: str) -> LoadTestResult:
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            "summary": {
                ...
                "overall_error_rate": sum(r.failed_requests for r in self.results) / sum(r.total_requests for r in self.results) * 100,
                "average_rps": statistics.mean([r.requests_per_second for r in self.results]),
                "average_response_time": statistics.mean([r.avg_response_time for r in self.results]),
            },
            "tests": []
        }
...
def run_locust_load_test(config: LoadTestConfig):
    ...
    cmd = [
        ...
        "--locustfile", locust_config["locustfile"],
        "--host", locust_config["host"],
        "--users", str(locust_config["users"]),
        "--spawn-rate", str(locust_config["spawn-rate"]),
        "--run-time", locust_config["run-time"],
        "--headless",
        "--html", locust_config["html"],
        "--csv", locust_config["csv"],
    ]
