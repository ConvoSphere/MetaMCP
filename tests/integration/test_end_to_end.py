...
class TestEndToEndWorkflows:
    ...
    async def test_complete_user_workflow(self, setup_services, async_client):
        ...
        login_response = await async_client.post(
            "/api/v1/auth/login", json={"username": "user", "password": "user123"}
        )
        ...
        register_response = await async_client.post(
            "/api/v1/tools/register", json=tool_data, headers=headers
        )
        ...
        tool_details_response = await async_client.get(
            f"/api/v1/tools/{tool_id}", headers=headers
        )
        ...
        update_response = await async_client.put(
            f"/api/v1/tools/{tool_id}", json=updated_tool_data, headers=headers
        )
    ...
    async def test_admin_workflow(self, setup_services, async_client):
        ...
        # 4. Get All Tools
        tools_response = await async_client.get(
            "/api/v1/admin/tools", headers=admin_headers
        )
    ...
    async def test_error_handling_workflow(self, setup_services, async_client):
        ...
        nonexistent_tool_response = await async_client.get(
            "/api/v1/tools/nonexistent-tool-id", headers=headers
        )
