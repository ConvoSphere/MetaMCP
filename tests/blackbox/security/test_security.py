...
class TestInputValidationSecurity:
    ...
    @pytest.mark.asyncio
    async def test_xss_injection_tool_registration(self, authenticated_client: AsyncClient):
        """Test XSS injection resistance in tool registration."""
        xss_payloads = [
            {"name": "<script>alert('xss')</script>", "description": "XSS test", "endpoint": "http://localhost:9000"},
            {"name": "test_tool", "description": "<img src=x onerror=alert('xss')>", "endpoint": "http://localhost:9000"},
            {"name": "test_tool", "description": "Normal description", "endpoint": "javascript:alert('xss')"},
        ]
    ...
    @pytest.mark.asyncio
    async def test_command_injection_tool_execution(self, authenticated_client: AsyncClient):
        ...
        for payload in injection_payloads:
            response = await authenticated_client.post(
                f"{API_BASE_URL}tools/test_tool/execute",
                json=payload
            )
