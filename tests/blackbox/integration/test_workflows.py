...
from ..conftest import API_BASE_URL, TEST_USER, TEST_TOOL
...
class TestUserWorkflows:
    ...
    async def test_complete_tool_lifecycle(self, http_client: AsyncClient):
        ...
        tool_data["name"] = f"workflow_test_tool_{int(pytest.time.time())}"
        ...
        execute_response = await http_client.post(
            f"{API_BASE_URL}tools/{tool_data['name']}/execute",
            json=execution_data
        )
        ...
        search_found = any(result["name"] == tool_data["name"] for result in search_results)
        assert search_found, f"Tool not found in search results: {tool_data['name']}"
        
        # Step 7: Delete tool
        delete_response = await http_client.delete(f"{API_BASE_URL}tools/{tool_data['name']}")
...
class TestConcurrentWorkflows:
    ...
    async def test_concurrent_user_sessions(self, http_client: AsyncClient):
        ...
        assert success_count >= 3, f"Only {success_count}/5 concurrent sessions succeeded"
    ...
    async def test_concurrent_tool_operations(self, http_client: AsyncClient):
        ...
        success_count = sum(1 for r in results if r is True)
        assert success_count >= 2, f"Only {success_count}/3 concurrent tool operations succeeded"
