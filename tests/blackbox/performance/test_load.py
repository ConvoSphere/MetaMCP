...
class TestPerformanceEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_tool_registration_performance(self, authenticated_client: AsyncClient):
...
class TestLoadTesting:
    ...
    async def test_mixed_workload(self, authenticated_client: AsyncClient):
        ...
        success_count = sum(1 for r in responses if not isinstance(r, Exception) and r.status_code in [200, 404])
        assert success_count >= len(responses) * 0.6, f"Too many failures: {success_count}/{len(responses)}"
...
class TestStressTesting:
    ...
    async def test_large_payload_handling(self, authenticated_client: AsyncClient):
        ...
        large_tool = {
            ...
            "endpoint": "http://localhost:9000",
            "category": "stress",
            "capabilities": ["read", "write", "execute"] * 100,  # Large capabilities list
            "security_level": 1,
