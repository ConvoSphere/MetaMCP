...
class TestToolRetrieval:
    ...
    async def test_get_tool_success(
        self, authenticated_client: AsyncClient, test_tool_id: str
    ):
...
class TestToolExecution:
    ...
    async def test_execute_tool_success(
        ...
        response = await authenticated_client.post(
            f"{API_BASE_URL}tools/{TEST_TOOL['name']}/execute", json=execution_data
        )
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_execute_tool_not_found(self, authenticated_client: AsyncClient):
        ...
        execution_data = {"arguments": {"param": "value"}}
        response = await authenticated_client.post(
            f"{API_BASE_URL}tools/nonexistent_tool/execute", json=execution_data
        )
        assert response.status_code == 404
        data = response.json()
        assert "error" in data or "detail" in data
    ...
    async def test_execute_tool_invalid_arguments(
        ...
        execution_data = {"arguments": {"invalid_param": "value"}}
        response = await authenticated_client.post(
            f"{API_BASE_URL}tools/{TEST_TOOL['name']}/execute", json=execution_data
        )
...
class TestToolManagement:
    ...
    async def test_update_tool(
        ...
        response = await authenticated_client.put(
            f"{API_BASE_URL}tools/{TEST_TOOL['name']}", json=update_data
        )
        assert response.status_code == 200
        data = response.json()
